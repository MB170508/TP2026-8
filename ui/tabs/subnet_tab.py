...
def create_subnet_tab(page: ft.Page) -> ft.Column:
    ...
    Provides VLSM (Variable Length Subnet Masking) subnet calculation
    with custom segment requirements.

    Args:
        page: Flet page instance (for page.update() calls)

    Returns:
        ft.Column containing complete subnet calculator tab UI
    """
    # ── Initialize ──────────────────────────────────
    ...
    network_input = ft.TextField(
        label="Network (e.g. 192.168.10.0/24)", width=280, content_padding=8
    )
    ...
    def build_segments():
        """Rebuild segments display from calculator state."""
    ...
    def add_segment(e):
        """Add a new segment to the calculator."""
        subnet_error.value = ""
    ...
    def remove_segment(idx):
        """Remove a segment from the calculator."""
    ...
    def calc_subnet(e):
        ...
        subnet_result_col.controls.append(
            ft.Text(
                f"Base: {result['base_address']}  |  Total IPs: {result['total_ips']}",
                size=13,
            ...
            subnet_result_col.controls.append(
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Text(
                                f"Seg {i} — {s['users']} users",
                                size=13,
                                weight=ft.FontWeight.W_600,
                            ),
                            ft.Text(
                                f"Network: {s['network']}/{s['cidr']}  |  Mask: {s['mask']}",
                                size=12,
                            ),
                            ft.Text(
                                f"Usable: {range_str} ({s['usable']} hosts)", size=12
                            ),
                        ],
                        spacing=2,
                    ),
    ...
    return ft.Column(
        [
            ...
            ft.Row(
                [btn("Calculate", calc_subnet, primary=True), btn("Reset", reset_subnet)]
            ),
            subnet_result_col,
        ],
